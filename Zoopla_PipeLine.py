...
def wrapper_update_delete_records(**kwargs):
    ...
    Update_Delete_Records.update_status(delete_records).Main()
...
with DAG('Zoopla_PipeLine',
         default_args={
             ...
             'start_date': datetime.today(),   # i don't define start date like that datetime(2025, 4, 18) becasue to prevent the scheduler from running the dag atomatically for the past dates every sunday becasue it shadual "weekly"
             'retries': 1,
             'catchup': False,
    ...
    Wait_Spider_Finish = FileSensor(
        ...
        filepath="/usr/local/airflow/include/zoopla_resources/output/zoopla_listing_data.json" ,  # Note : this path is relative to the docker container
